...
gs = gridspec.GridSpec(2, 2, hspace=0.0000000000001, wspace=0.1)
fig = plt.figure(figsize=(5, 5.7), frameon=False)
...
ax1.contourf(int_global[0], int_global[1], int_global[2][:, :].T, clevs,
             cmap=cmap)
...
int_clusters = get_tif_array_crop('envmodsoft/output/' +
                                  'tair_20161213_max_clusters.tif')
...
int_global = get_tif_array_crop('envmodsoft/output/' +
                                'tair_20161213_min_noclusters.tif')
im1 = ax3.contourf(int_global[0], int_global[1], int_global[2][:, :].T, clevs,
                   cmap=cmap)
...
int_clusters = get_tif_array_crop('envmodsoft/output/' +
                                  'tair_20161213_min_clusters.tif')
ax4.contourf(int_clusters[0], int_clusters[1], int_clusters[2][:, :].T, clevs,
             cmap=cmap)
ax4.set_aspect('equal')
ax4.axis('off')
ax4.text(271030, 4742408, 'd)', size=3)

clusters_6.plot(ax=ax4, facecolor='None', edgecolor='black', linewidth=0.3)

cbar_ax = fig.add_axes([0.35, 0.25, 0.30, 0.015])
cbar = plt.colorbar(im1, cax=cbar_ax, orientation='horizontal')
cbar.ax.tick_params(labelsize=3, width=0.3, size=2, pad=1)
cbar.outline.set_linewidth(0.3)
cbar.set_label('Air temperature (\u00b0C)', size=3)

plt.subplots_adjust(left=0.12, bottom=0.28, right=0.85, top=0.92, wspace=0.01, hspace=0.01)
plt.savefig('/tmp/ems_interpolation_comparison_minmax.pdf', bbox_inches='tight', pad=0)
